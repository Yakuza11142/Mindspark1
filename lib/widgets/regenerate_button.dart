...
class RegenerateButton extends StatelessWidget {
  ...
  const RegenerateButton(
      {super.key, required this.onForceNew, required this.assetType});
  ...
  Widget build(BuildContext context) {
    return ElevatedButton.icon(
      ...
      label: Text("Generate New $assetType (Costs 50 ⚡)",
      ...
      onPressed: () async {
        ...
        if (success) {
          ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text("Generating fresh asset...")));
          onForceNew(); // Triggers the HiveMindRouter with forceNewMovement = true
        } else {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
              content: Text("Not enough Sparks! Watch an Ad.",
