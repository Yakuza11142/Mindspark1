...
class DynamicGlobalPaywall extends StatefulWidget {
  ...
  const DynamicGlobalPaywall(
      {super.key, required this.userCountryIso, required this.targetExam});
...
class _DynamicGlobalPaywallState extends State<DynamicGlobalPaywall> {
  ...
  void _calculateLocalPrice() async {
    double usdPrice =
        PppPricingEngine.getOptimalPriceUsd(widget.userCountryIso);
      ...
      body: Center(
        child: Column(
          ...
          children: [
            ...
            Text("CRUSH THE ${widget.targetExam}",
                style: const TextStyle(
                    fontSize: 28,
                    fontWeight: FontWeight.bold,
                    color: Colors.white)),
            const Text("Unlock Pro AI & 3D Holo-Deck",
                style: TextStyle(color: Colors.grey)),
            const SizedBox(height: 40),
            Text(localPriceString,
                style: const TextStyle(
                    fontSize: 36,
                    color: Colors.amber,
                    fontWeight: FontWeight.bold)),
