...
class _ScienceLabScreenState extends State<ScienceLabScreen>
  ...
  void _toggleVibration() async {
    if (_isVibrating) {
      ...
      if (await Vibration.hasCustomVibrationsSupport() ?? false) {
        Vibration.vibrate(
            pattern: [500, 1000, 500, 2000], repeat: 0); // Heartbeat pattern
      ...
      bottomNavigationBar: BottomNavigationBar(
        ...
        items: const [
          BottomNavigationBarItem(
              icon: Icon(Icons.change_history), label: "Hologram"),
          BottomNavigationBarItem(
              icon: Icon(Icons.lightbulb), label: "Color Caster"),
          BottomNavigationBarItem(
              icon: Icon(Icons.vibration), label: "Frequency"),
        ],
      ),
      body: _buildBody(),
    );
  ...
  Widget _buildBody() {
    switch (_mode) {
      case 0:
        return _buildHologram();
      case 1:
        return _buildColorCaster();
      case 2:
        return _buildVibrationLab();
      default:
        return Container();
    }
  }
    ...
    return Stack(
      children: [
        ...
        Positioned(
            ...
            left: 0,
            right: 0,
            child: _rotatingObj(math.pi)), // Top (Inverted)
        Positioned(
            left: 50,
            top: 0,
            bottom: 0,
            child: _rotatingObj(math.pi / 2)), // Right
        Positioned(
            right: 50,
            top: 0,
            bottom: 0,
            child: _rotatingObj(-math.pi / 2)), // Left
        const Center(
            child: Text("+",
                style: TextStyle(
                    color: Colors.white, fontSize: 20))), // Center Marker
      ],
    );
  }
    ...
    return Transform.rotate(
      ...
      child: RotationTransition(
        ...
        child: const Icon(Icons.public,
            size: 80, color: Colors.blueAccent), // Replace with 3D Image
      ),
    );
  }
      ...
      child: Column(
        ...
        children: [
          Icon(Icons.speaker_phone,
              size: 100, color: _isVibrating ? Colors.redAccent : Colors.grey),
          const SizedBox(height: 20),
          const Text("Resonance Engine",
              style: TextStyle(color: Colors.white, fontSize: 24)),
          const Text("Place phone on a hollow box to hear the sound.",
