...
class ARHumanoid {
  ...
  void buildFullBody(v.Vector3 origin) {
    // --- TORSO & BACK ---
    _addNode(
        v.Vector3(0, 0.4, 0) + origin,
        ArCoreCylinder(materials: [glow], radius: 0.15, height: 0.5),
        "stomach_back");
  ...
  void _buildLimb(v.Vector3 origin,
      {required bool isLeg, required bool isLeft}) {
    double side = isLeft ? -1 : 1;
    double yPos = isLeg ? 0.15 : 0.6; // Shoulder vs Hip height

    // Joint 1: Shoulder/Hip
    _addNode(v.Vector3(side * 0.2, yPos, 0) + origin,
        ArCoreSphere(materials: [glow], radius: 0.05), "joint_start");
    ...
    // Joint 2: Elbow/Knee
    _addNode(v.Vector3(side * 0.2, yPos - 0.4, 0) + origin,
        ArCoreSphere(materials: [glow], radius: 0.04), "joint_mid");
    ...
    // Joint 3: Hand/Foot
    _addNode(v.Vector3(side * 0.2, yPos - 0.75, 0) + origin,
