  ...
  Future<String?> generate3D(String prompt) async {
    try {
      final res = await http.post(
          ...
          headers: {
            "Authorization": "Bearer ${Secrets.tripoKey}",
            "Content-Type": "application/json"
          },
          body: jsonEncode({
            "type": "text_to_model",
            "prompt": "A 3D model of $prompt, glb format"
          }));
  ...
  Future<String?> _poll(String id) async {
    for (int i = 0; i < 20; i++) {
      ...
      final res = await http.get(
          Uri.parse("https://api.tripo3d.ai/v2/openapi/task/$id"),
          headers: {"Authorization": "Bearer ${Secrets.tripoKey}"});
