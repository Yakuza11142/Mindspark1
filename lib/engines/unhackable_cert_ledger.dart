...
class UnhackableCertLedger {
  ...
  static Future<String?> mintCertificate(
      String studentName, String course, int score) async {
    try {
      // Create a secure record in Google's servers
      DocumentReference docRef =
          await _db.collection('verified_certificates').add({
  ...
  static Future<Map<String, dynamic>?> verify(String certId) async {
    try {
      DocumentSnapshot doc =
          await _db.collection('verified_certificates').doc(certId).get();
