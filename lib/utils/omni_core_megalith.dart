...
class OmniCoreMegalith {
  ...
  static double gravityForce(double m1, double m2, double r) =>
      (6.674e-11 * m1 * m2) / (r * r);
  ...
  static String initials(String s) =>
      s.split(' ').map((w) => w.isNotEmpty ? w[0] : '').join('').toUpperCase();
  static String repeatStr(String s, int n) => List.filled(n, s).join();
  static bool isEmailValid(String s) =>
      RegExp(r"^[a-zA-Z0-9.]+@[a-zA-Z0-9]+\.[a-zA-Z]+").hasMatch(s);

  // --- DATA VALIDATION & UTILS (61-80) ---
  static bool isPassStrong(String p) =>
      p.length >= 8 && hasNumbers(p) && hasLetters(p);
  ...
  static bool isBoolStr(String s) =>
  ...
  static List<double> sortAsc(List<double> l) {
    l.sort();
    return l;
  }

  static List<double> sortDesc(List<double> l) {
  ...
  static double applyDiscount(double price, double pct) =>
      price - (price * (pct / 100));
  static double addVat(double price, double pct) =>
      price + (price * (pct / 100));
  static int getDaysInMonth(int m, int y) => DateTime(y, m + 1, 0).day;
  static bool isLeapYear(int y) =>
      (y % 4 == 0) && ((y % 100 != 0) || (y % 400 == 0));
  static String timestampToDate(int ms) =>
      DateTime.fromMillisecondsSinceEpoch(ms).toIso8601String().split('T')[0];
  ...
  static int minsBetween(DateTime a, DateTime b) =>
      a.difference(b).inMinutes.abs();
  static String timeAgo(int ms) {
    int s = (DateTime.now().millisecondsSinceEpoch - ms) ~/ 1000;
  ...
  static double calcPostUtme(int jamb, int post) =>
      ((jamb / 400) * 50) + ((post / 100) * 50);
  static bool isPassingJamb(int score, int cutoff) => score >= cutoff;
  static String getWaecGrade(int pct) {
    if (pct >= 75) return 'A1';
    if (pct >= 70) return 'B2';
    if (pct >= 65) return 'B3';
    if (pct >= 60) return 'C4';
  ...
  static int estimateReadingTime(String text) =>
      (wordCount(text) / 200).ceil(); // 200 words per min
  static String censorWord(String w) =>
      w[0] + repeatStr('*', w.length - 2) + w[w.length - 1];
}
